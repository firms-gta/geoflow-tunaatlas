...
list(
  tar_target(
    config,
    initWorkflow(here::here("config/create_effort_dataset.json"))  # 🔥 recréer `config` depuis le fichier
  ),
  ...
  tar_target(
    output_file,
    {
    path <- executeWorkflow(here::here("config/create_effort_dataset.json"))  # 🔥 recréer `config` depuis le fichier
      entity_tar <- entity
...
#   tar_target(
#     config,
#     initWorkflow(here::here("config/catch_ird_level2_local.json")),  # 🔥 Chargement direct de l’objet R6

...
if(step_title_t_f){cat_title(step_title, child_headerinside = child_header)}
...
```{r results='asis'}
if(!unique_analyse && parameter_short == FALSE ){cat_title("# Main differences \n ", child_headerinside = child_header)}
```
...
```{r results='asis'}
if(!unique_analyse){cat_title("# Introduction to the two datasets")}
```
...
```{r results='asis'}
cat_title("# Differences between the two datasets", child_headerinside = child_header)
```

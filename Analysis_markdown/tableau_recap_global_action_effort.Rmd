...
child_env_last$parameter_init <- paste0(sub_list_dir_2[length(sub_list_dir_2)], "/rds.csv")
...
child_env_comp$parameter_init <- paste0(sub_list_dir_2[1], "/rds.csv")
child_env_comp$parameter_final <- paste0(sub_list_dir_2[length(sub_list_dir_2)], "/rds.csv")
...
df <- data.frame(matrix(ncol =13, nrow = 1))
      colnames(df) <- c(paste0(tail(str_split(paste0(sub_list_dir_2),"/")[[1]],n=1)),
          ...
          main <- filtering_function(fread(paste0(sub_list_dir_2[1],"/rds.csv")), parameter_filtering = parameter_filtering)
      ...
      for (i in sub_list_dir_2){
        ...
        nrow <- sums$lines
          ...
          main <- filtering_function(fread(paste0(i,"/rds.csv")), parameter_filtering = parameter_filtering)
...
df <- data.frame(matrix(ncol =13, nrow = 1))
      colnames(df) <- c(paste0(tail(str_split(paste0(sub_list_dir_2),"/")[[1]],n=1)),
          ...
          main <- filtering_function(fread(paste0(sub_list_dir_2[1],"/rds.csv")), parameter_filtering = parameter_filtering)# %>% dplyr::filter(species%in%opts$species_filter)
      ...
      for (i in sub_list_dir_2){
        ...
        if (file.exists(paste0(i,"/options_written.txt"))){
          Options <- pull(readtext(paste0(i,"/options_written.txt"))[2])} else {Options <- "Aucune"}
          main <- filtering_function(fread(paste0(i,"/rds.csv")), parameter_filtering = parameter_filtering)# %>% dplyr::filter(species%in%filter_species)
...
child_env$parameter_init <- paste0(sub_list_dir[counting], "/rds.csv")
child_env$parameter_final <- paste0(sub_list_dir[counting+1], "/rds.csv")
...
if( identical(fread(child_env$parameter_init),fread(child_env$parameter_final) )){NA}else{

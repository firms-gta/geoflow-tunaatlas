...
parameters_child <- list(config = config, entity = entity,opts = opts ,action = action, parameter_filtering = parameter_filtering, parameter_fact = parameter_fact, parameter_con = con,  child = TRUE, no_recap_options = TRUE, plotting_type = "plot",parameter_colnames_to_keep = c("fishing_fleet",         "gear_type",                 "time_start",                 
                      ...
                      "measurement_unit",                 "measurement_value",                "source_authority", "GRIDTYPE", "Gear", "species_group"), treatment = FALSE, shapefile.fix = shapefile.fix, shape_without_geom = shape_without_geom, continent = continent, fig.path = fig.path, outputonly = FALSE, print_map = TRUE, coverage = FALSE)
...
new_path <- file.path(fig.path, "/Comparison/initfinal_",gsub("Markdown/","",sub_list_dir_2[1]),"_", gsub("Markdown/","",sub_list_dir_2[length(sub_list_dir_2)]))

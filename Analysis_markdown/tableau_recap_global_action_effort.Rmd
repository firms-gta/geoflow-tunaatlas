...
log_file <- paste0(getwd(), "/file.log")
...
if(dir.exists("Markdown")){
  ...
  if(parameter_fact == "catch"){
    nominal_dataset <- readr::read_csv(paste0("data/global_nominal_catch_firms_level0.csv"))}else if (parameter_fact == "effort"){
    nominal_dataset <- readr::read_csv(paste0("data/global_nominal_catch_firms_level0.csv"))
    } 
  
  else {nominal_dataset <- readr::read_csv(paste0("data/global_nominal_catch_firms_level0.csv"))}
...
dir.create(paste0(fig.path, "Report_on_", entity$identifiers[["id"]], "/figures/"), recursive = TRUE)

child_env_last$fig.path = paste0(fig.path,"Reporton", entity$identifiers[["id"]], "/figures/")
...
```{r complastfirst, echo=FALSE, results='asis'}
...
df <- data.frame(matrix(ncol =13, nrow = 1))
      ...
      for (i in sub_list_dir_2){
        ...
        if(length(parameter_filtering) == 0){
...
qflextable2(reduced, captionn ="Evolution of Tons and Number of fish and lines along the steps", pgwidth = 6, save_folder = "Summary", columns_to_color = c("Difference (in % of tons)","Difference (in % of fish)"), fig.pathinside = getwd())
```
...
```{r evolcapturesduringprocess, echo=FALSE, fig.cap='Evolution of captures in tons and number of fish during the process',results='asis', eval=(parameter_fact == "catch")}

save_image(cowplot::plot_grid(tons_plot, no_fish_plot))
cowplot::plot_grid(tons_plot, no_fish_plot)

```

```{r evlocapturesduringprocesseffort, echo=FALSE, fig.cap='Evolution of captures in fishing days and hooks during the process',results='asis', eval=(parameter_fact == "effort")}

save_image(cowplot::plot_grid(fishing_days_plot, hooks_plot))
...
child_env$fig.path = paste0(fig.path, "Comparison/",gsub("Markdown/","",sub_list_dir[counting]),"_", gsub("Markdown/","",sub_list_dir[counting+1]), "/figures/")

...
column_names = colnames(invalid_data)

# Remove '.rds' from each column name
cleaned_column_names <- gsub(".rds", "", column_names)

colnames(invalid_data) <- cleaned_column_names
...
source("https://raw.githubusercontent.com/firms-gta/geoflow-tunaatlas/Developpement/Analysis_markdown/functions/Functions_markdown.R")
issued_dataframes <- qflextable2(invalid_data %>% dplyr::filter(All_good == FALSE) %>% dplyr::select(-All_good), captionn = "Recap of the issues with each dataframe", grouped_data = c( "tRFMO"))
for (i in columns_to_color){
  ft$body$styles$text$color$data[,i] <- ifelse(is.na(ft$body$dataset[,i]), "blue", ifelse(ft$body$dataset[,i], "red", "green"))
...
recap_mapping <- readRDS(file.path(path,"all_recap_mapping.rds")) %>%ungroup() %>% arrange(source_authority)%>% dplyr::select(-Entity)  %>% filter(src_code != trg_code) %>% distinct()

...
'', 
  ...
  if (all(class(x) == "flextable")) {
    ...
    if (is.character(column) | is.factor(column)) {
      ...
      column <- stri_replace_all_regex(column, "[^\\p{ASCII}]", "")
    }
    return(column)
  })
...
tidying_data_for_GTA = function(dataframe, time_dim, shape = NULL, species_group_dataframe = NULL, cl_cwp_gear_level2_dataframe = NULL, parameter_colnames_to_keep_dataframe = parameter_colnames_to_keep){

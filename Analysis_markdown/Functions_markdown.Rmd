...
tidying_data <- function(dataframe, parameter_colnames_to_keep_dataframe = parameter_colnames_to_keep, time_dim){
...
knitting_plots_subfigures <- function(plot, title, general_title = NULL, folder = "Unknown_folder", fig.pathinside = fig.path) {
  save_image(title = title, plott = plot, folder = folder, fig.pathinside = fig.pathinside)
    assign("title", gsub("_", "-",title))

  # chunk_header <- paste0('```{r ', title, ', fig.cap="', title, '", fig.align="center", out.width="100%", results="asis"}')
  # chunk_footer <- '```'

  knitr::knit_child(text = c(
    '```{r plottingmultiples, results = "asis" ,fig.cap =`title`}',
    '',
    '',
    'plot(plot)',
    '',

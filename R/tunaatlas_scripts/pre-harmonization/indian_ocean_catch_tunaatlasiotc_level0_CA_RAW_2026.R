#' Harmonize IOTC Catch Dataset (standardized formatting)
#'
...
capt <- capt_raw %>%
  dplyr::mutate(
    ...
    fishing_mode = as.character(CATCH_SCHOOL_TYPE_CODE),
    # core dimensions
    ...
    # measure
    measurement_value = CATCH,
    measurement_unit = case_when(
      CATCH_UNIT_CODE == "NO" ~ "no",
      CATCH_UNIT_CODE == "MT"~ "t",
      TRUE ~ "unknown"
    ),
    fishing_mode = case_when(
      CATCH_SCHOOL_TYPE_CODE == "FS" ~ "FS",
      CATCH_SCHOOL_TYPE_CODE == "LS" ~ "LS",
      TRUE ~ "UNK"
    ),
  ...
  dplyr::select(
    ...
    measurement_value, measurement_unit, species
  ) %>%
...
capt$time_start <- as.Date(capt$time_start)
capt$time_end   <- as.Date(capt$time_end)
capt$measurement_processing_level <- "unknown"
capt$measurement_type <- "RC" 
capt$measurement <- "catch" 

dataset_temporal_extent <- paste(
  paste0(format(min(capt$time_start, na.rm = TRUE), "%Y"), "-01-01"),
  paste0(format(max(capt$time_end, na.rm = TRUE), "%Y"), "-12-31"),
  sep = "/"
)

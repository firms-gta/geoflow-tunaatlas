create_plsql_data_getter <- function(action,entity, config){
  ...
  sql_drop <- sprintf("DROP FUNCTION IF EXISTS public.get_fact_dataset_%s(%s)", fact, paste0(rep("character varying", length(dimensions)+3),collapse=","))
  ...
  sql_create <- paste(sql_create,
		...
		END; $$ 
	LANGUAGE plpgsql;")
  log_info("Create or Replace PL/PSQL function to get dataset")
  log_info(paste("\n", sql_create))

map_codelist = function (df_input, df_mapping, dimension_to_map, keep_src_code = FALSE) 
{
  ...
  if (keep_src_code == FALSE) {
    df_input <- df_input %>% group_by_(.dots = setdiff(column_names_df_input, 
                                                       "value")) %>% summarise(value = sum(value))
  }
  df_input <- data.frame(df_input)
  stats_data_not_mapped <- df_input %>% mutate(sum_mapped_unmapped = ifelse(is.na(df_input[, 
  ...
  stats_data_not_mapped <- dcast(setDT(stats_data_not_mapped), 

...
knitr::opts_chunk$set(
  ...
  message = FALSE,
  warning = FALSE,
  fig.path = "figures/summarycatchlevel2-"
)
...
## Reading the harmonized file from level 2 workflow output

```{r downloadingcsv}

csv_file <- here::here("data/global_catch_ird_level2_1950_2023_harmonized.csv")
...
tapply(catch_data$measurement_value, catch_data$measurement_unit, summary)
...
qs_file <- here::here("data/UN_CONTINENT2.qs")
...
silky_sharks <- catch_data %>% dplyr::filter(species == "SKH")
...
groupped_gear <- silky_sharks %>% dplyr::group_by(gear_type, time_start, measurement_unit) %>% 
...
groupped_fishing_fleet <- silky_sharks %>% dplyr::group_by(fishing_fleet, time_start, measurement_unit) %>% 
  dplyr::summarise(measurement_value = sum(measurement_value))
...
source("https://raw.githubusercontent.com/firms-gta/geoflow-tunaatlas/master/R/tunaatlas_scripts/generation/spatial_curation_downgrade_resolution.R")

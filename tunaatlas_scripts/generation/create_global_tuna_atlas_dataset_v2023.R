function(action, entity, config) {
  ...
  url_scripts_create_own_tuna_atlas <-
    "https://raw.githubusercontent.com/eblondel/geoflow-tunaatlas/master/tunaatlas_scripts/generation"
  source(file.path(
    url_scripts_create_own_tuna_atlas,
    "get_rfmos_datasets_level0.R"
  )) #modified for geoflow
  source(file.path(
    url_scripts_create_own_tuna_atlas,
    "retrieve_nominal_catch.R"
  )) #modified for geoflow
  ...
  source(file.path(
    url_scripts_create_own_tuna_atlas,
    "function_overlapped.R"
  )) # adding this function as overlapping is now a recurent procedures for several overlapping
  source(
    file.path(
      url_scripts_create_own_tuna_atlas,
      "disaggregate_on_resdeg_data_with_resolution_superior_to_resdeg.R"
    )
  )
  source(
    file.path(
      url_scripts_create_own_tuna_atlas,
      "function_raising_georef_to_nominal.R"
    )
  ) #modified for geoflow
  source(
    file.path(
      url_scripts_create_own_tuna_atlas,
      "spatial_curation_data_mislocated.R"
    )
  ) #modified for geoflow
  source(file.path(
    url_scripts_create_own_tuna_atlas,
    "double_unit_data_handling.R"
  )) # new function for double unit
  source(file.path(
    url_scripts_create_own_tuna_atlas,
    "function_recap_each_step.R"
  ))  # new function to create rds for each treatment
  ...
  config$logger.info(
    "LEVEL 0 => STEP Retrieve georeferenced catch or effort (+ processings for IATTC) AND NOMINAL CATCH if asked"
  )
  #-------------------------------------------------------------------------------------------------------------------------------------
  ...
  # ------------filtering on minimum time to get all the data ending at the same year-------------------------------------------------------------
  
  
  
  georef_dataset <- time_extent_harmonisation(georef_dataset)
  ...
  if (!is.null(opts$mapping_map_code_lists))
    if (opts$mapping_map_code_lists) {
      ...
      mapping_csv_mapping_datasets_url <-
        entity$getJobDataResource(config, entity$data$source[[2]])
      mapping_dataset <-
        read.csv(
          mapping_csv_mapping_datasets_url,
          stringsAsFactors = F,
      ...
      if (!is.null(opts$mapping_keep_src_code))
        mapping_keep_src_code = opts$mapping_keep_src_code
      ...
      mapping_codelist <-
        map_codelists(
          ...
          source_authority_to_map = opts$source_authority_to_map
...
list(options_mapping_map_code_lists)
  ...
  # Filtering on species for level 0 --------------------------------------
  georef_dataset <- filter_species_from_asfis_list(config, georef_dataset, level = "0")
  ...
  config$logger.info("LEVEL 0 => STEP Overlapping zone (IATTC/WCPFC): keep data from IATTC or WCPFC?")
  #-----------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  config$logger.info("LEVEL 0 =>  Overlapping zone (IOTC/WCPFC): keep data from IOTC or WCPFC?")
  #-----------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  sbf_declaration <- setdiff(unique((georef_dataset %>% dplyr::filter(species == "SBF"))$source_authority), "CCSBT")
  
  lapply(sbf_declaration, FUN =  function_duplicate_sbf, georef_dataset = georef_dataset, connection = con,  options_strata_overlap_sbf = opts$strata_overlap_sbf, opts_sbf = opts)
  ...
  #-----------------------------------------------------------------------------------------------------------------------------------------------------------
  if (!is.null(opts$aggregate_on_5deg_data_with_resolution_inferior_to_5deg))
    if (opts$aggregate_on_5deg_data_with_resolution_inferior_to_5deg) {
      config$logger.info(
  ...
  if (!is.null(opts$unit_conversion_convert)){
    if (opts$unit_conversion_convert) {
      georef_dataset <-
      ...
      config$logger.info(
        sprintf(
          "LEVEL 1 => Convert units by using A. Fonteneau file. Option is: [%s] ",
          entity$data$source[[1]],
      ...
      config$logger.info(
        sprintf(
          " Gridded catch dataset before unit conversion has [%s] lines and total catch is [%s] Tons",
          nrow(georef_dataset),
          ntons_before_this_step
        )
      )
      
      config$logger.info(": BEGIN do_unit_conversion() function to convert units of georef_dataset")
      ...
      config$logger.info("END do_unit_conversion() function")
      
      ntons_after_conversion <-
        round(georef_dataset %>% select(value)  %>% sum())
      config$logger.info(
        sprintf(
          "Gridded catch dataset after unit conversion has [%s] lines and total catch is [%s] Tons",
          nrow(georef_dataset),
          ntons_after_conversion
        )
      )
      ...
      config$logger.info(
        sprintf(
          "Total number for 'NO' unit is now [%s] individuals",
      ...
      config$logger.info(
        sprintf(
          "LEVEL 1 => not executed  for file [%s] (since not selected in the workflow options, see column 'Data' of geoflow entities spreadsheet): Convert units by using A. Fonteneau file. Option is: [%s] ",
          entity$data$source[[1]],
          opts$unit_conversion_convert
        )
      )
  ...
  # Filtering on species for level 1 --------------------------------------
    config$logger.info("Filtering on species for level 0")
    georef_dataset <- filter_species_from_asfis_list(georef_dataset, level = "1")
  ...
  config$logger.info("LEVEL 2 Extract and load IRD Level 1 gridded catch data input")
  config$logger.info(
    "-----------------------------------------------------------------------------------------------------"
  )
  ...
  if (!is.null(opts$raising_georef_to_nominal))
    if (opts$raising_georef_to_nominal) {
      ...
      config$logger.info(
        "LEVEL 2 Raise IRD gridded Level 1 (1 or 5 deg) input with FIRMS Level O total (nominal) catch dataset"
      )
      config$logger.info(
        "-----------------------------------------------------------------------------------------------------"
      )
  ...
  if (opts$disaggregate_on_5deg_data_with_resolution_superior_to_5deg %in% c("disaggregate", "remove")) {
    ...
    config$logger.info(
      sprintf(
        "LEVEL 1 => for file [%s] is executed: Disaggregate data on 5° resolution quadrants (for 5deg resolution datasets only). Option is: [%s] ",
        entity$data$source[[1]],
    ...
    config$logger.info(
      " function_disaggregate_on_resdeg_data_with_resolution_superior_to_resdeg() function"
    )
  ...
  gc()
   ...
   #-----------------------------------------------------------------------------------------------------------------------------------------------------------
  config$logger.info("Step: Grid spatial resolution filter")
  #-----------------------------------------------------------------------------------------------------------------------------------------------------------
  if (!is.null(opts$resolution_filter)) {
    ...
    if (nrow(geograph_identifier) != nrow(georef_dataset)) {
      ...
      georef_dataset <-
        georef_dataset[startsWith(georef_dataset$geographic_identifier,
                                  opts$resolution_filter), ]
  ...
  if (!is.null(opts$gear_filter)) {
    ...
    georef_dataset <-
      georef_dataset %>% dplyr::filter(gear %in% gear_filter)
  ...
  #-----------------------------------------------------------------------------------------------------------------------------------------------------------
  config$logger.info("Step Apply filters if filter needed (Filter data by groups of everything) ")
  #-----------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  filtering_function = function(dataframe_to_filter,
    ...
    if (length(matchingList) != 0) {
      ...
      function_recap_each_step(
        "Filtering",
        georef_dataset,
        "This step is to filter on multiple columns",
        "",
        list(parameter_filtering)
      )
    } else{
      dataframe_to_filter
    }
  ...
  dataset <-
    georef_dataset %>% group_by(.dots = setdiff(colnames(georef_dataset), "value")) %>% dplyr::summarise(value =
                                                                                                           sum(value))
  ...
  output_name_dataset <-
    file.path("data", paste0(entity$identifiers[["id"]], "_harmonized.csv"))
  ...
  output_name_codelists <-
    file.path("data", paste0(entity$identifiers[["id"]], "_codelists.csv"))

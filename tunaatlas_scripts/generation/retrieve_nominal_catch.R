retrieve_nominal_catch <- function(entity, config, options){
	...
	#Note: for the timebeing ICCAT aligned on providing data by stocks in the CWP RH format
	...
	columns_to_keep=c("source_authority","species","gear_type","fishing_fleet","time_start","time_end","geographic_identifier","measurement_unit","measurement_value")
	
	nominal_catch <- as.data.frame(do.call("rbind", lapply(dataset_files_nominal_catch, function(x){ readr::read_csv(x, guess_max = 0) })))

...
cwp_grid_from_latlon <- function(df_input, colname_latitude, colname_longitude, colname_squaresize) {
  ...
  # Adjust latitude and longitude based on quadrant to reference the correct grid corner
  df_input <- df_input %>%
    mutate(
      adjusted_lat = case_when(
        quadrant %in% c(1, 4) ~ floor(!!sym(colname_latitude) / !!sym(colname_squaresize)) * !!sym(colname_squaresize),
        quadrant %in% c(2, 3) ~ ceiling(!!sym(colname_latitude) / !!sym(colname_squaresize)) * !!sym(colname_squaresize)
      ),
      adjusted_lon = case_when(
        quadrant %in% c(1, 2) ~ floor(!!sym(colname_longitude) / !!sym(colname_squaresize)) * !!sym(colname_squaresize),
        quadrant %in% c(3, 4) ~ ceiling(!!sym(colname_longitude) / !!sym(colname_squaresize)) * !!sym(colname_squaresize)
      )
    )
  ...
  df_input <- df_input %>%
    ...
    select(-c(quadrant, adjusted_lon, adjusted_lat, grid_code))  # Remove intermediate columns

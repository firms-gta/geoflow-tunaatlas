...
deploy_database_model <- function(config, software, software_config){
	...
	for (i in 1:length(dimensions)){
	  ...
	  dimensionTableComment <- switch(dimension,
	                       "time" = "create_schema_dimension_time.sql",
	                       "sizeclass" = "create_schema_dimension_sizeclass.sql",
	                       "catch" = paste0("COMMENT ON TABLE ",dimension,".",dimension," IS 'Quantity of fish  (in number or weight / biomass) harvested in a given stratum.';"),
	                       "effort" = paste0("COMMENT ON TABLE ",dimension,".",dimension," IS 'Quantity of effort (expressed in a given unit such as number of sets, number of fishing hours, number of hooks, etc.) exerted in a given stratum.';"),
	                       "catch-at-size" = paste0("COMMENT ON TABLE ",dimension,".",dimension," IS 'Quantity of fish at a given size (or size class) harvested in a given stratum.';"),
	                       ...
	                       "catchtype" = paste0("COMMENT ON TABLE ",dimension,".",dimension," IS 'Fate of the catch, i.e. landed, discarded, unknown. Given the nature of the data, only landing data are currently available in SARDARA, with a very few exceptions of discarded fishes for ICCAT.';"),
	                       "catchunit" = paste0("COMMENT ON TABLE ",dimension,".",dimension," IS 'Unit of catch (metric tons or number of fish). Catches are mainly given in weight (metric tons), but some historical catch (e.g. japanese longliners) are reported in number of fishes captured. Some catch are also reported in both weight and number.';"),
	                       "effortunit" = paste0("COMMENT ON TABLE ",dimension,".",dimension," IS 'Unit of effort. The unit of effort mainly depends on the gear and the country that reports the data. Several units of efforts are available, even for a given gear (e.g. day at sea and searching days for purse seine).';"),
	                       "flag" = paste0("COMMENT ON TABLE ",dimension,".",dimension," IS 'Flagging country of the fishing vessels. Data are generally reported by country but some data can be reported at a sub-level, e.g. catch from Reunion Island longliners are reported under the REU flag and not FRA.';"),
	                       "gear" = paste0("COMMENT ON TABLE ",dimension,".",dimension," IS 'Fishing gear used. The number of gears varies a lot depending on the RFMOs. ICCAT, for instance, has around 60 gears while IATTC has 10 gears.';"),
...
sex	Sex of the fish captured. Only partially available in the catch-at-size datasets.';"),
	                       "species" = paste0("COMMENT ON TABLE ",dimension,".",dimension," IS 'Species captured. The main tuna species are available in all the RFMOs Depending on the RFMO, some non-target species (e.g. some sharks or turtles) are also reported.';"),
	                       "sizeclass" = paste0("COMMENT ON TABLE ",dimension,".",dimension," IS 'Class of size of a distribution. This dimension is used only in the catch-at-size dataset. A class is defined by its minimum size (e.g. 30 cm) and the size bin (e.g. 2 cm).';"),
	                       "time" = paste0("COMMENT ON TABLE ",dimension,".",dimension," IS 'Dating of the fact. In Sardara, the dating is provided as two columns: time_start gives the first date of availability of the measure (included) and time_end gives the last date of availability of the measure (not included). The data in Sardara are mainly defined over the following time steps: 1) Nominal catch are mostly defined on 1 year resolution. 2) Georeferenced catch-and-effort and catch-at-size are mostly defined on 1 month resolution';")
	  )
	  ...
	  outsql <- paste0(outsql, dimensionTableComment,"\n")
